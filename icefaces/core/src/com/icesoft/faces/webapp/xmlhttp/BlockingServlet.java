...
public class BlockingServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request,
        ...
        if( session == null ) {
            respondSessionExpired( response );
            return;
        }
        ...
        try {
            ...
            viewNumber = (String) session.getAttribute(
        ...
        try {
            ...
            if ( ("/send-updates".equals(method)) || 
                ...
                synchronized(eventLock) {
                    executeRequest(request, response, state, viewNumbers);
                }
            } else {
                executeRequest(request, response, state, viewNumbers);
            }
            ...
            // We wont catch this unless a redirect has been requested
            ...
            //attempt to detect session expiry explicitly.
            if (log.isTraceEnabled()) log.trace(
                    "User session expired. ViewNumber = [" + viewNumber + "]", e);
            respondSessionExpired( response );
    ...
    private void executeRequest(HttpServletRequest request, 
            HttpServletResponse response, ResponseState state,
            String[] viewNumbers) throws IOException, IllegalAccessException {
    ...
    private void respondSessionExpired(HttpServletResponse response)
        throws IOException
    {
    ...
    /**
     * Convert a Map of String -> String[] entries into a <code>Map</code>
     * of either <code>String -> List</code> entries if the value has more
     * than one entry, or a Map of <code> String -> String</code> entries if
     * the value is a single String
     *
     * @param parameters The original map
     * @return A new Map containing the converted parameters
     */
    ...
    private void receiveUpdates(HttpServletRequest request,
        ...
        if ("true".equals(request.getParameter("partial"))) {
            ...
            UIComponent component = D2DViewHandler
                    .findComponent(componentID, context.getViewRoot());            
